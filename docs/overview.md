...
- Game page flow: `next-app/app/game/[versionId]/page.tsx` validates version/bundle availability, renders `GameApp`, and mounts `GamePageClientBootstrap` to start `/games/:versionId/dist/game.js` plus admin/dev client behaviors through Next-managed dynamic imports; bootstrap manages one active runtime teardown at a time and exposes a host-invokable global teardown handle.
