...
- Always-open game-page prompt composer with keyboard submit (`Enter`) plus optional browser speech-to-text recording that flashes while active and inserts transcript text when recording is toggled off.
...
- `src/` - Backend app, HTML rendering, browser assets, and service modules.
  ...
  - `public/` - Static browser assets.
    - `styles.css` - Homepage/game-page styling, always-open top prompt composer bar (with pulsing recording state), desktop split-panel game layout, and transcript layouts.
    - `game-view.js` - Prompt submit + speech-to-text recording UX, game-page transcript polling, and transcript auto-scroll behavior.
...
- Render surface model: desktop game pages use a fixed full-width top prompt composer (input + right-side `Record` button), with a split layout beneath it (left fixed-aspect `9:16` game stage + right full-height Codex transcript panel); under `980px`, the transcript panel is hidden and the game remains portrait-centered below the prompt composer.
...
- Test coverage focus: route rendering/order, `/codex` page/API behavior and missing-state handling, shared transcript presenter usage paths, desktop game split-layout markup/CSS hooks, always-open prompt composer + record-button behavior, Codex JSONL parsing/lookup, version metadata parsing (including `codexSessionId`), fork lineage/copy behavior (including ID collision retry/validation), prompt composition, build pipeline command sequencing, dev reload-token writing, game-page live-reload polling behavior, and repository automation contracts (`conductor.json` + deploy workflow key commands).
- End-to-end/manual flow: run `npm run dev`, open `/`, verify reverse-chronological 3-column tiles; open `/game/v1-bounce` on desktop and verify the always-open top prompt composer (`Record` on the right), `Enter` submit behavior, speech-record flashing while active, transcript insertion when recording toggles off, and left game + right full-height transcript panel auto-scroll; open `/game/elm-cloud-sage` and verify a dense field of shimmering bowls with animated iridescence/specular highlights remains readable in the portrait render area; open `/codex`, choose a version, and verify user/assistant transcript rendering or the correct missing-session state.
