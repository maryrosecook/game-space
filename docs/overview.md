...
- Manual tile snapshot flow: admin game pages expose a blue recorder button (`#game-tab-capture-tile`) that captures current `#game-canvas` pixels as a PNG data URL; `POST /api/games/:versionId/tile-snapshot` validates the PNG payload and writes `games/<version-id>/snapshots/tile.png` so homepage tiles can immediately use the captured image.
...
- Manual tile snapshot model: admins can save the current rendered game canvas as `snapshots/tile.png` through `/api/games/:versionId/tile-snapshot`; homepage tile media discovery picks up this standard snapshot path without requiring metadata backfills.
