...
- `games/` - Versioned game sandboxes (one runtime/build boundary per version).
  - `starter/` - Minimal touch-first WebGL starter powered by `src/engine/*` (`engine.ts`, `input.ts`, `blueprints.ts`, `particles.ts`, `physics.ts`, `render.ts`, `types.ts`) plus sample wiring in `src/main.ts`; intentionally omits GUI/editor features and ships local `typecheck` support via `tsc --noEmit`.
...
- Prompt fork flow: `POST /api/games/:versionId/prompts` requires admin + CSRF, forks via `createForkedGameVersion()` (prompt-derived three-word base plus random 10-character lowercase alphanumeric suffix), sets lifecycle state to `created`, composes full prompt, launches the provider-selected runner (`codex` or `claude`) behind `CodexRunner`, persists `codexSessionId` as soon as observed, and transitions lifecycle to `stopped` or `error` when the run settles.
