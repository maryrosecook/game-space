...
- Starter now supports both deterministic headless runs (Playwright + SwiftShader flags) driven by a bounded JSON action protocol (`run` / `input` / `snap`) and metadata-driven runtime settings via persisted per-version slider controls (`particles` in the starter proof-of-concept, with `gameDevRequested` tracking on slider metadata).
