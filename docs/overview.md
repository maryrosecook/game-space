...
- Ideas flow: `/ideas` requires admin; `next-app/app/ideas/page.tsx` renders `IdeasPageClient` with base-game selector data (starter + favorited versions, including tile snapshots), and client logic uses typed API wrappers for generate/build/delete while preserving CSRF/header semantics, selected base-game ideation context, and redirect-on-build behavior.
...
- Coverage focus:
  ...
  - `/ideas` client behavior for base-game selector UI and generate request payload (`baseGameVersionId`).
