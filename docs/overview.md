...
- Starter now supports both deterministic headless runs (Playwright + SwiftShader flags) driven by a bounded JSON action protocol (`run` / `input` / `snap`), metadata-driven runtime settings via persisted per-version slider controls (`particles` in the starter proof-of-concept, with `gameDevRequested` tracking on slider metadata), and lineage-aware clone history for starter-derived forks.
...
- `src/` - Backend runtime, browser assets, and service modules.
  ...
  - `services/` - Filesystem, auth, build, provider-configurable prompt execution, and session/transcript orchestration.
    ...
    - `gameVersions.ts` - Version ID validation, metadata parsing/writing (including `favorite`, `lineageId`, and optional creation `prompt`), lifecycle status normalization, and version listing.
    - `gameLineages.ts` - Starter-root lineage resolution/grouping helpers shared by homepage tiles, game-page history UI, and delete handling.
    - `forkGameVersion.ts` - Fork copy plus persisted lineage metadata creation/inheritance, excluding copied snapshot artifacts.
...
- `src/app/` - Next.js App Router surface for Next-owned runtime routes.
  ...
  - `shared/` - Shared Next-owned client/server page modules.
    - `components/` - Route-level React UI trees used by Next SSR + client hydration.
      - `GameLineageModal.tsx` - Server-rendered lineage history modal content for the admin game page.
    ...
    - `homepagePageData.ts` - Homepage lineage-tile/auth-link mapping helper.
...
- `game-build-prompt.md` - Prompt prelude prepended to user prompt text before Codex execution, including runtime-settings guidance for `globals`, `editor.sliders`, `gameDevRequested`, and `[game-dir]/control-state.json`.
...
- Auth/homepage/API flow: Next handles `/`, `/_next/*`, `/auth*`, `/api/*`, `/game/*`, `/codex`, `/ideas`, `/public/*`, `/games/*`, and `/favicon.ico`; the homepage now groups starter-derived clone trees into one tile per lineage using `gameLineages.ts`, while Node-runtime handlers preserve existing CSRF, rate-limit, cookie, and admin-404 contracts.
- Game page flow: `src/app/game/[versionId]/page.tsx` validates version/bundle availability, renders `GameApp`, reads any persisted `control-state.json`, resolves same-lineage clone entries for admin sessions, and mounts `GamePageClientBootstrap` to start `/games/:versionId/dist/game.js` plus admin/dev client behaviors through Next-managed dynamic imports; bootstrap manages one active runtime teardown at a time, exposes a host-invokable global teardown handle, and passes runtime-settings host capabilities (`versionId`, control-state load/save) into `startGame(canvas, host?)`.
- Runtime settings flow: admin game pages now split bottom-drawer UX into hammer/build tools and cog/settings drawers; `game-view-client.js` reads runtime slider metadata from `window.__gameSpaceActiveGameRuntimeControls`, keeps the Settings tab disabled until at least one runtime slider exists, renders range inputs into `#settings-form`, keeps the settings drawer sized to its current content up to one-third of the viewport, and debounces `serializeControlState()` saves to `POST /api/games/:versionId/control-state`, while both admin and public loads merge persisted globals from `games/<versionId>/control-state.json`.
...
- Lineage history flow: admin game pages expose a clock action in the Edit drawer; server-rendered lineage rows come from `gameLineages.ts`, and `game-view-client.js` opens/closes the modal, navigates to selected clones, and deletes clone directories through `DELETE /api/games/:versionId` while redirecting to another same-lineage clone (or home when none remain) if the active clone is deleted.
...
- Prompt fork flow: `POST /api/games/:versionId/prompts` requires admin + CSRF, forks via `createForkedGameVersion()` (prompt-derived three-word base plus random 10-character lowercase alphanumeric suffix), persists the submitted user prompt plus stable `lineageId` in the new fork's `metadata.json`, sets lifecycle state to `created`, composes full prompt, launches the provider-selected runner (`codex` or `claude`) behind `CodexRunner`, persists `codexSessionId` as soon as observed, and transitions lifecycle to `stopped` or `error` when the run settles.
...
- `games/` - Primary persisted store (no external database).
  - `<version-id>/`
    - `metadata.json` - `{ id: string, parentId: string | null, lineageId?: string, createdTime: ISO-8601 string, threeWords?: string, prompt?: string, tileColor?: "#RRGGBB", favorite?: boolean, codexSessionId?: string | null, codexSessionStatus?: "none" | "created" | "stopped" | "error", tileSnapshotPath?: string }`.
...
- Runtime settings model: game runtimes can declare top-level `globals` plus `editor.sliders`; each slider metadata entry now includes `gameDevRequested`, the starter bootstrap exposes `getSliders`, `setGlobalValue`, and `serializeControlState`, and Next persists overrides per version in `control-state.json` without rewriting source files while keeping the admin Settings tab disabled for games that expose no sliders.
...
- Lineage persistence model: starter prompts create a new stable `lineageId` equal to the first fork id; later forks inherit that lineage id, homepage grouping uses it when available (falling back to parent-chain resolution for older metadata), and clone deletion backfills `lineageId` onto surviving same-lineage members before removing the deleted directory so grouping still works after lineage roots are removed.
...
- Fork snapshot reset model: `createForkedGameVersion()` copies source files into the new worktree but skips the top-level `snapshots/` directory, so cloned homepage tiles cannot inherit stale `snapshots/tile.png` output from the source game before a new capture is generated.
...
- Starter game base model: `games/starter/src/engine/engine.ts` runs lifecycle/tick phases with adapter injection points for input and frame scheduling. Browser gameplay defaults to `BrowserInputManager` + `BrowserRafScheduler`; headless runs inject `HeadlessInputManager` + `HeadlessFrameScheduler` while leaving game/blueprint logic unchanged. `games/starter/src/main.ts` now ships one hidden particle-emitter blueprint/thing, declares `globals.particles` plus `editor.sliders`, and uses that persisted global as the multiplier for falling yellow/orange/red particles.
...
- Automatic tile snapshot protocol model: `promptSubmission.ts` drives post-generation tile capture after any successful codegen exit, using a headless protocol that runs gameplay frames, injects synthetic touch down/up input, then performs a final `snap` step before persisting `snapshots/tile.png`.
...
- Homepage lineage-tile layout model: `.game-grid` uses `repeat(3, minmax(0, 1fr))` so sparse tile sets still render in at least three columns; `.game-tile` media is full-bleed (`.tile-image` absolute inset), starter-derived clone trees collapse to one representative tile per lineage, and game names are exposed through tile link accessibility labels.
- Lineage modal model: admin game pages render a fixed-position backdrop/modal above the bottom drawers; `game-view-client.js` treats outside taps, Escape, and the close button as dismiss actions, disables the play button for the active clone, and keeps per-row delete actions wired to the same server delete endpoint as the main trash button.
...
- Coverage focus:
  ...
  - Lineage grouping coverage for homepage representative-tile selection, stored-lineage fallback after root deletion, and clone-history modal navigation/delete behavior.
  ...
  - Runtime-settings coverage for slider metadata validation/merge logic, `/api/games/:versionId/control-state` validation + atomic persistence, hammer/settings drawer behavior, icon-only microphone button, and starter `particles` persistence across reloads.
...
- End-to-end/manual flow:
  ...
  - Verify lineage behavior: prompting from `starter` creates one homepage tile for that lineage (latest visible representative for the current auth state), the Edit drawer clock button opens clone history, play jumps between same-lineage clones, and deleting the active clone redirects to another same-lineage clone or `/` when none remain.
